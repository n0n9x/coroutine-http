...
{
    int flags = fcntl(fd, F_GETFL, 0);      // 拿到这个 fd 现有的所有状态标志
    fcntl(fd, F_SETFL, flags | O_NONBLOCK); // 按位“或”上一个非阻塞标志位
}
...
{
    listen_fd_ = socket(AF_INET, SOCK_STREAM, 0); // 代表这个空的socket的文件描述符
    if (listen_fd_ < 0)
    ...
    addr.sin_port = htons(port_);      // 设置端口号，并进行字节序转换
    addr.sin_addr.s_addr = INADDR_ANY; // 指定监听的 IP 地址为 “任意地址”
...
{
    if (!handler_)
        throw std::runtime_error("must call on_connect() first");
    init_socket();
    accept_sched_.create([this]()
                         { accept_loop(); });
}
...
void TcpServer::accept_loop()
{
    while (true)
    {
        ...
        while (true)
        {
            ...
            {
                Scheduler &s = *tl_scheduler;  // 取当前线程的 Scheduler
                Connection conn(client_fd, s); // 创建 Connection，绑定到当前线程的 Scheduler
                handler_(move(conn));          // 调用 handle_connection(conn)
            };
